...
export default function Sidebar({
  ...
  const currentView = location.pathname.split("/")[1] || "oficios";
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-slate-200 transform transition-transform duration-200 ease-in-out md:translate-x-0 flex flex-col h-screen ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex items-center justify-between h-16 px-6 border-b border-slate-200">
          ...
          {isOpen && (
            <button
              onClick={onClose}
              className="md:hidden p-2 -mr-2 text-slate-500 hover:text-slate-700 rounded-lg cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          )}
                  ...
                  <div key={item.name} className="flex flex-col">
                    ...
                    >
                      <div className="flex items-center">
                        <item.icon
                          className={`mr-3 h-[18px] w-[18px] transition-colors ${isCurrent || isChildActive ? "text-emerald-600" : "text-slate-400 group-hover:text-slate-600"}`}
                        />
                        {item.name}
                      </div>
                      {hasChildren && (
                        <ChevronRight
                          className={`w-4 h-4 transition-transform duration-200 ${isExpanded ? "rotate-90 text-slate-400" : "text-slate-300 group-hover:text-slate-400"}`}
                        />
                      )}
                    </button>
                        ...
                        <div className="ml-5 pl-4 border-l border-slate-100 space-y-1 py-1 relative">
                          {item.children?.map((child) => {
                            ...
                            return (
                              ...
                              >
                                <div
                                  className={`absolute -left-[3px] w-1.5 h-1.5 rounded-full transition-colors ${
                                    isChildCurrent
                                      ? "bg-emerald-500"
