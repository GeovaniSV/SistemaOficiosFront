...
export function OficioPreviewModal({
  isOpen,
  onClose,
  oficio,
  ...
  const responsible = oficio.responsibles?.[selectedResponsibleIndex];
    ...
    >
      <div className="bg-slate-50 -mx-6 -mt-6">
        <div className="p-4 sm:p-8 overflow-y-auto">
          {oficio.destination_contact &&
            Array.isArray(oficio.destination_contact) &&
            oficio.destination_contact.length > 1 && (
              <div className="mb-6 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSelectedDestinatarioIndex(Number(e.target.value))
                  }
                ...
                >
                  {oficio.destination_contact.map((dest, index) => (
          ...
          {oficio.responsibles && oficio.responsibles.length > 1 && (
            <div className="mb-6 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
              <label className="block text-sm font-medium text-slate-700 mb-2">
                Visualizar versão para o responsável:
              </label>
              <select
                value={selectedResponsibleIndex}
                onChange={(e) =>
                  setSelectedResponsibleIndex(Number(e.target.value))
                }
                className="block w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 sm:text-sm transition-colors"
              >
                {oficio.responsibles.map((resp, index) => (
                  <option key={index} value={index}>
                    {resp.treatment ? `${resp.treatment} ` : ""}
                    {resp.name}
                    {resp.department ? ` — ${resp.department}` : ""}
                  </option>
                ))}
              </select>
            </div>
          ...
          {/* {isRecebido && oficio. ? (
            <div className="bg-white border text-center border-slate-200 shadow-sm aspect-video flex items-center justify-center relative overflow-hidden h-[700px]">
              <object
                data={oficio.fileData}
                type="application/pdf"
                width="100%"
                height="100%"
              >
                <p>
                  Parece que o seu navegador não suporta PDFs renderizados
                  inline.{" "}
                  <a
                    href={oficio.fileData}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-blue-500 underline"
                  >
                    Clique aqui para bater o download
                  </a>
                  .
                </p>
              </object>
          ...
          <React.Fragment>
            <div className="bg-white border border-slate-200 shadow-sm min-h-[600px] p-6 sm:p-10 flex flex-col relative w-full max-w-3xl mx-auto">
              ...
              {oficio.status === "Aprovado" && (
                ...
                >
                  Documento assinado digitalmente por Reginaldo Monteiro de
                  Oliveira, Presidente, em{" "}
                  {new Date(oficio.created_at).toLocaleDateString("pt-BR")}.
                  Hash:
                  e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855.
                  Valide em oficiopro.com.br/validacao com o código:
                  A8F9-B2C4-E7D1-55XQ
                </div>
              )}
              <DocumentHeader />
              ...
              <div className="bg-white border border-slate-200 shadow-sm min-h-[600px] p-6 sm:p-10 flex flex-col mt-8">
                <div className="border-b-2 border-slate-800 pb-4 mb-8 text-center">
                  <h2 className="text-lg sm:text-xl font-bold uppercase">
                    PROTOCOLO DE ASSINATURA ELETRÔNICA
                  </h2>
                ...
                <div className="flex-1 space-y-8">
                        ...
                        <div>
                          ...
                          <div className="mt-3 space-y-1 text-xs text-slate-500">
                            <p>
                              ...
                              {new Date(oficio.created_at).toLocaleDateString(
                                "pt-BR",
                              )}
                            </p>
                            <p>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-700 leading-relaxed">
                      A autenticidade deste documento e de suas assinaturas pode
                      ser verificada acessando o portal de validação através do
                      link abaixo:
                    </p>
                    <div className="mt-4 p-4 bg-slate-100 rounded-lg flex items-center justify-between">
                      <div>
                        <a
                          href="#"
                          className="text-blue-600 hover:underline text-sm font-medium break-all block mb-2"
                        >
                          https://oficiopro.com.br/validacao
                        </a>
                        <p className="text-sm text-slate-700">
                          Informe o seguinte código:{" "}
                          <span className="font-mono font-bold bg-white px-2 py-1 rounded border border-slate-200 text-slate-900 tracking-widest ml-1">
                            A8F9-B2C4-E7D1-55XQ
                          </span>
                        </p>
                      </div>
